...
class Song {
  ...
  howMany(newListeners) {
    const newListenersLowerCase = newListeners.map((listener) =>
      listener.toLowerCase()
    );
    let numOfNewListeners = 0;

    for (const newListener of newListenersLowerCase) {
      if (!this.currentListeners.includes(newListener)) {
        this.currentListeners.push(newListener);
        numOfNewListeners++;
      }
    }

    return numOfNewListeners;
  }
}
